...
class AudioService: # AudioService classımız 
 ...
 ######///////////////////////////////////////////////////////////////////////////////////////////////       
        while True: # while döngüsü
            ...
            elif 'sağlık kimlik' or 'sağlık bilgi'in str(getQuery).lower():
                self.speakAudio("kan grubu yaş kronik hastalık boy kilo eklendi")

                #####################
                #
                # sağlık sistemi yakın zamanda güncellenecek
                #
                #####################


            elif 'misis kent' in str(getQuery).lower(): 
                self.speakAudio("buyrun efendim ben misis kent") 
            
            elif 'youtube aç'or'youtube' in str(getQuery).lower():       ###### diğer fonksiyonla
                self.speakAudio("youtube açılıyor") 
                wbb.open("https://www.youtube.com/?hl=TR")
            
            elif 'google aç'or'google' in str(getQuery).lower(): 
                self.speakAudio("google açılıyor")
                wbb.open("https://www.google.com/webhp?hl=tr&sa=X&ved=0ahUKEwjA2c6Jp9L4AhX8if0HHV5dC6EQPAgI")

            elif 'insta'or 'instgram aç' in str(getQuery).lower(): 
                self.speakAudio("instgram açılıyor")
                wbb.open("https://www.instagram.com/?hl=tr")
   

            elif 'nasılsın'or'naber' in str(getQuery).lower():
                self.speakAudio("iyim sizler nasılsınız ")
            elif 'misis'or'kent' in str(getQuery).lower():
                self.speakAudio("buyrun")          
            elif 'imdat'or'yardım' in str(getQuery).lower():
                self.speakAudio("155 ve 112  arandı yardım geliyor ")
            ############ 
            # 
            # bluetooth özellikleri çalışan güncelenecek
            # 
            # ##########  

            elif 'rapor et'or 'rapor' in str(getQuery).lower(): 

                self.speakAudio("rapor ediliyor")
                ########
                #
                # rapor edilime func günlenecektir
                #
                ######

                
            elif 'discord'or 'discord aç' in str(getQuery).lower(): 
                self.speakAudio("discord açılıyor")
                wbb.open("https://discord.com/")     

            else:
                self.speakAudio("özellik dışı üzgünüm en kısa zamanda eklenecektir lütfen rapor et diyin yakında androide taşınıyoruz")


            

##//////////////////////////////////////////////////////////////////// PENCEREMİZİ AYARLIYORUZ
...
class TkinterGUI: # TkinterGUI classımız
    ...
    def __init__(self): # TkinterGUI classımızın constructorı
        ...
        self.root.title("               MRS KENT") # title değişkeni oluşturuyoruz
        ...
        self.frameCnt = 75 # frameCnt değişkeni oluşturuyoruz
        ...
        self.root.iconbitmap(r"D:\Users\NURİ ÖZTÜRK\Desktop\New folder (2)\icon.ico")#  Window için İcon koyduk isterseniz değiştiriniz
...
### BY Özcan ÖZTÜRK 
#
#PyQt5 Desingerla Pencersi Düzenlenebilir foksiyon ve animsyon ekleyebilirsiniz